...
namespace Lacrima
{
    ...
    public:
        ...
        void addPoint(int64_t id, const glm::vec2& position, float weight_scale = 1.0f);
        void removePoint(int64_t id);
        bool hasPoint(int64_t id) const;

        glm::vec2 getPointPosition(int64_t id) const;
        void setPointPosition(int64_t id, const glm::vec2& position);
        ...
        int64_t getClosestPoint(const glm::vec2& position, bool include_disabled = false) const;
        glm::vec2 getClosestPositionInSegment(const glm::vec2& position) const;

        std::vector<glm::vec2> getPointPath(int64_t from_id, int64_t to_id, bool allow_partial_path = false);
